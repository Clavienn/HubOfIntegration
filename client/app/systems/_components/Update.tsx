...
import { System, UpdateSystem, UpdateSystemSchema } from '@/domains/models/System';
...
interface Props {
  ...
  open: boolean;
  onClose: () => void;
  onUpdated: (updated: System) => void; // FIX: retourne le System mis à jour
}

export default function SystemUpdate({ system, open, onClose, onUpdated }: Props) {
  const [serverError, setServerError] = useState<string | null>(null);

  const { register, handleSubmit, reset, setValue, watch,
    formState: { errors, isSubmitting, isDirty } } = useForm<UpdateSystem>();
  ...
  const onSubmit = async (raw: UpdateSystem) => {
    ...
    try {
      const updated = await systemRepo.update(system.id, data); // FIX: on récupère le System mis à jour
      onUpdated(updated);
      onClose();
    } catch (err: unknown) {
  ...
  const handleClose = () => { setServerError(null); onClose(); };
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader><DialogTitle>Modifier — {system.name}</DialogTitle></DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <Switch id="isActive" checked={isActive ?? false}
